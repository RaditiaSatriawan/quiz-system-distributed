...
from models import init_db, create_quiz, get_quiz, get_all_quizzes, update_quiz, delete_quiz, create_question, get_questions_by_quiz
...
def rpc_create_quiz():
    try:
        ...
        quiz = create_quiz(title=title, description=data.get('description'), time_limit_minutes=data.get('time_limit_minutes', 30))
...
def rpc_update_quiz(quiz_id):
    try:
        data = request.get_json(force=True)
        quiz = update_quiz(quiz_id, title=data.get('title'), description=data.get('description'), time_limit_minutes=data.get('time_limit_minutes'))
        if not quiz:
            return _jsonify({'error': 'Quiz not found'}, 404)
...
def rpc_add_question(quiz_id):
    try:
        ...
        question = create_question(quiz_id=quiz_id, question_text=data['question_text'], option_a=data['option_a'], option_b=data['option_b'], option_c=data['option_c'], option_d=data['option_d'], correct_answer=correct, points=data.get('points', 10))
