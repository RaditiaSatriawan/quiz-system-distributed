...
def _get_connection():
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            logger.warning('RabbitMQ connection attempt %d failed: %s – retrying in %ds', attempt, exc, RETRY_DELAY_SECONDS)
...
def _publish_notification(channel, submission_id, grading_result):
    try:
        ...
        notification = {'event': 'grading_complete', 'submission_id': submission_id, 'student_name': submission['student_name'] if submission else 'unknown', 'quiz_id': submission['quiz_id'] if submission else None, 'quiz_title': grading_result.get('quiz_title', 'Unknown Quiz'), 'score': grading_result.get('score'), 'correct_count': grading_result.get('correct_count'), 'total_questions': grading_result.get('total_questions'), 'status': grading_result.get('status', 'graded')}
        channel.queue_declare(queue=NOTIFICATION_QUEUE, durable=True)
        channel.basic_publish(exchange='', routing_key=NOTIFICATION_QUEUE, body=json.dumps(notification), properties=pika.BasicProperties(delivery_mode=2))
...
def publish_grading_task(submission_id):
    ...
    try:
        ...
        channel.basic_publish(exchange='', routing_key=GRADING_QUEUE, body=message, properties=pika.BasicProperties(delivery_mode=2))
